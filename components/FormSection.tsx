...
interface FormSectionProps {
  form: {
    ...
    priGunu: number;
    borçlanmaDahil: boolean; // YENİ: dahil/hariç tik
  ...
  onBorclanmaDahilChange: (dahil: boolean) => void; // YENİ
  onHesapla: () => void;
}
  ...
  return (
    <div className="card sticky top-4 max-h-[calc(100vh-2rem)] overflow-y-auto">
          ...
          {[
            ...
            <label key={stat.value} className="flex items-center cursor-pointer">
              <input
                ...
                name="statular"
                value={stat.value}
                checked={form.statular[0] === stat.value}
      ...
      {form.statular.length > 0 && (
        <div className="mb-6 p-4 bg-green-50 rounded-lg border border-green-300">
              ...
              {form.malulBirimi === 'sk28/5' && (
                <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded">
                  <label className="block text-sm font-medium text-gray-900 mb-2">
                    Engelli Derece <span className="text-red-500">*</span>
                  </label>
                  <select
                  ...
                  {errors.malulDerece && <p className="text-xs text-red-600 mt-1">{errors.malulDerece}</p>}
                      ...
                      <label className="flex items-center cursor-pointer">
                        ...
                        <span className="ml-2 text-sm text-gray-700">
                          ...
                          <span className="text-xs text-gray-500 block">
                            (Başkasının bakımına muhtaçsanız işaretleyin)
                          </span>
                        </span>
                      </label>
                    </div>
          ...
          {form.statular[0] === '2925' && (
            <p className="text-sm text-gray-500">2925 statüsünde malüllük koşulu bulunmamaktadır.</p>
          )}
        </div>
      )}
      ...
      <div className="mb-4">
        ...
        />
        {errors.dogumTarihi && <p className="text-xs text-red-600 mt-1">{errors.dogumTarihi}</p>}
      </div>
      ...
      <div className="mb-4">
        ...
        />
        {errors.ilkIsGirisTarihi && <p className="text-xs text-red-600 mt-1">{errors.ilkIsGirisTarihi}</p>}
      </div>

      {/* PRİM GÜNÜ + BORÇLANMA TİK */}
      <div className="mb-4">
        <label className="block text-sm font-medium text-gray-700 mb-1">
          Prim Günü
        </label>
        ...
        />
        {/* Borçlanma dahil/hariç — prim günü altında 2 küçük tik */}
        <div className="flex gap-4 mt-2">
          <label className="flex items-center gap-1.5 cursor-pointer text-xs text-gray-600">
            <input
              type="radio"
              name="borclanmaDahil"
              checked={!form.borçlanmaDahil}
              onChange={() => onBorclanmaDahilChange(false)}
          ...
          <label className="flex items-center gap-1.5 cursor-pointer text-xs text-gray-600">
            <input
              ...
              name="borclanmaDahil"
              checked={form.borçlanmaDahil}
              onChange={() => onBorclanmaDahilChange(true)}
              className="w-3.5 h-3.5 text-blue-600"
            />
            Borçlanma dahil
          </label>
        ...
        <p className="text-xs text-gray-400 mt-1">
          {form.borçlanmaDahil
            ? 'Askerlik günleri prim gününe dahil sayılacak'
            : 'Askerlik günleri prim gününe ayrıca eklenecek'}
        </p>
      </div>
        ...
        <div className="space-y-2">
          ...
          <label className="flex items-center cursor-pointer">
            ...
            <span className="ml-2 text-sm text-gray-700">
              ...
              <span className="text-xs text-gray-500 ml-1">(Sadece gün eklenir)</span>
            </span>
          </label>
        </div>
